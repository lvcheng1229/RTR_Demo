...
/**
  ...
  rawNormalLength Length of the normal before normalization, based on MIP-mapping of the normal: a measure of smoothness under MIP-mapping
...
/**
  ...
  */
void bumpMapBlinn78(in sampler2D normalBumpMap, in float bumpMapScale, in float bumpMapBias, in vec2 texCoord, in vec3 tan_X, in vec3 tan_Y, in vec3 tan_Z, in float backside, in vec3 tsE, out vec3 wsN, out vec2 offsetTexCoord, out vec3 tsN, out float rawNormalLength) {
        
    offsetTexCoord = texCoord;
            
    // Take the normal map values back to (-1, 1) range to compute a tangent space normal
    tsN = texture2D(normalBumpMap, offsetTexCoord).xyz * 2.0 + vec3(-1.0);
...
/**
    ...
    */
void bumpMapWelsh04(in sampler2D normalBumpMap, in float bumpMapScale, in float bumpMapBias, in vec2 texCoord, in vec3 tan_X, in vec3 tan_Y, in vec3 tan_Z, in float backside, in vec3 tsE, out vec3 wsN, out vec2 offsetTexCoord, out vec3 tsN, out float rawNormalLength) {
...
/**
    ...
    */
void bumpMapTatarchuk06(in sampler2D normalBumpMap, in float bumpMapScale, in float bumpMapBias, in vec2 texCoord, in vec3 tan_X, in vec3 tan_Y, in vec3 tan_Z, in float backside, in vec3 tsE, out vec3 wsN, out vec2 offsetTexCoord, out vec3 tsN, out float rawNormalLength) {
    ...
    // As we approach the full-res MIP map, let the rawNormal length be 1.0.  This avoids having
    // bilinear interpolation create short normals under the magnification filter
    rawNormalLength = lerp(1.0, length(NB.xyz * 2.0 - 1.0), min(1.0, computeSampleRate(offsetTexCoord, textureSize(normalBumpMap, 0))));
