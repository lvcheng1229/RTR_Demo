...
public:
	...
	float u_LightBias = 0.008f;
	float u_LightNear = 2.0f;
	float u_LightFar = 7.5f;
	float u_LightSize = 0.07f;
	float u_BlockerSearchScale = 1.0f;

	int u_BlockerSearchSamples = 25;
	int u_PCFSamples = 25;
	int u_blocker_search_samples_idx = 1;
	...
	{			
		ShadowMapShader =new Shader("../../Core/PCSS/ShadowMapShader.vert", "../../Core/PCSS/ShadowMapShader.frag");

		SecondShader =new Shader("../../Core/PCSS/secondShader.vert", "../../Core/PCSS/secondShader.frag");
		...
		SecondShader->setInt("s_ShadowMap", 1);
	...
	void update()
	{
		...
		lightProjection = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, u_LightNear, u_LightFar);
		...
		SecondShader->setInt("u_BlockerSearchSamples", u_BlockerSearchSamples);
		SecondShader->setInt("u_PCFSamples", u_PCFSamples);
		if (u_PCFSamples == u_BlockerSearchSamples)
		{
			setPossionSample(*SecondShader, u_BlockerSearchSamples);
		}
		else
		{
			setPossionSample(*SecondShader, u_BlockerSearchSamples);
			setPossionSample(*SecondShader, u_PCFSamples);
		}
		SecondShader->setMat4("light_view_proj", lightSpaceMatrix);
		SecondShader->setMat4("light_view", lightView);

		SecondShader->setFloat("u_LightBias", u_LightBias);
		SecondShader->setFloat("u_BlockerSearchScale", u_BlockerSearchScale);
		SecondShader->setFloat("u_LightSize", u_LightSize);
		SecondShader->setFloat("u_LightNear", u_LightNear);
		SecondShader->setFloat("u_LightFar", u_LightFar);
	...
	void DrawUI()
	{
		...
		ImGui::Begin("PCSS", &show_window, ImGuiWindowFlags_MenuBar);
		ImGui::SliderFloat("Light Size", &u_LightSize, 0.0f, 1.0f);
...

		{
			...
			const int   sample_counts[] = { 4,25, 32, 64, 100, 128 };
			ImGui::ListBox("Blocker Search Samples", &u_blocker_search_samples_idx, listbox_items, IM_ARRAYSIZE(listbox_items), 6);
			ImGui::ListBox("PCF Samples", &u_pcf_filter_samples_idx, listbox_items, IM_ARRAYSIZE(listbox_items), 6);

			u_BlockerSearchSamples = sample_counts[u_blocker_search_samples_idx];
			u_PCFSamples     = sample_counts[u_pcf_filter_samples_idx];
		}

		ImGui::End();
	}
	void setPossionSample(Shader &shader, int sampleNum)
	{
		using namespace glm;
		if (sampleNum == 25)
		{
			...
			{
				shader.setVec2("Poisson25[" + to_string(i) + "]", Poisson25[i]);
			}
		}
		else if (sampleNum == 4)
		{
			...
			for (int i = 0; i < sampleNum; i++)
			{
				shader.setVec2("Poisson4[" + to_string(i) + "]", Poisson4[i]);
			}
		}
		else if (sampleNum == 32)
		{
			vec2 Poisson32[32] = {
				...
				for (int i = 0; i < sampleNum; i++)
				{
					shader.setVec2("Poisson32[" + to_string(i) + "]", Poisson32[i]);
				}
		}
		else if (sampleNum == 64)
		{
			vec2 Poisson64[64] = {
			...
			for (int i = 0; i < sampleNum; i++)
			{
				shader.setVec2("Poisson64[" + to_string(i) + "]", Poisson64[i]);
			}
		}
		else if (sampleNum == 100)
		{
			vec2 Poisson100[100] = {
				...
				for (int i = 0; i < sampleNum; i++)
				{
					shader.setVec2("Poisson100[" + to_string(i) + "]", Poisson100[i]);
				}
		}
		else if (sampleNum == 128)
		{
			vec2 Poisson128[128] = {
		...
		for (int i = 0; i < sampleNum; i++)
		{
			shader.setVec2("Poisson128[" + to_string(i) + "]", Poisson128[i]);
		}
		}
	}
};
